...
final class CallAnnouncer {
    private let synthesizer: AVSpeechSynthesizer = {
        ...
        //
        // 照顧者端的警報工作階段有特定設定（`.playback`、依情況 duck），而
        // 合成器若自建工作階段，說完後會把它停用，可能連帶影響我們的設定。
        // 明示共用一個，行為才可預測。
        //
        // ⚠️ 本設定加入時的理由是「實測發現背景時第一則有聲音、之後都聽不到」，
        // **那個歸因後來證實是錯的**——真正的原因是 App 播完音檔就被凍結
        // （見 `DECISIONS.md` 2026-08-12）。設定本身仍然正確且該保留，
        // 但它並不是當初那個症狀的解方。
