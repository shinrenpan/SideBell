...
4. **中文版避開「44」的時間** ⚠️
   ...
   拍攝時看一下時鐘。`14:44`、`04:44` 這類時間出現在畫面上，對華語圈的讀者
   是忌諱，而這份文件的讀者是重症病友與他們的家人。

   這不是美感問題。第一版的 `c-10-lockscreen` 大鐘就是 14:44，是一位病友
   看了之後提出來的——**開發者不會自己想到，因為那不是他每天在面對的事。**

   受影響的是所有會顯示時間的畫面，其中最顯眼的兩張：
   `c-10-lockscreen`（鎖定畫面的大鐘）與 `c-11-call-list`（呼叫時間）。
