...
## Elevator pitch（一句話，上限約 200 字元）

```
An accessible call bell that works without the internet. One tap on the patient's iPad; the caregiver's phone raises an alarm and says aloud what is needed. Bluetooth only — no server, no account.
```
...
```
swift, swiftui, uikit, core-bluetooth, swiftdata, revenuecat, xcodegen, ios
```

## Try it out

```
https://apps.apple.com/app/id6799201744
https://github.com/shinrenpan/SideBell
```

## Video demo

```
https://youtu.be/cQijN-efZqE
```

## Image gallery

依序上傳（`build/screenshots/`，若已清除可跑 `./scripts/screenshots.sh` 重產）：

1. `ipad-patient-en.png` — 患者端，主圖
2. `iphone-caregiver-en.png` — 照顧者端，三種狀態同框
3. `iphone-patient-en.png` — 患者端在 iPhone 上
4. `ipad-caregiver-en.png` — 照顧者端在 iPad 上

---

## 規則要求對照

規則原文在 `rules.txt`（PDF 轉出）。逐條核對：

**資格 —— 全部符合**

- [x] 用 RevenueCat SDK 驅動 IAP（三個 consumable）
- [x] iOS
- [x] 美國可用（145 個地區含 USA）
- [x] 首個公開版本在 7/31–9/30 期間發布（8/27）
- [x] 不是既有 App 的更新

**提交素材**

- [x] 文字描述 —— 本檔案
- [x] Demo 影片 < 2 分鐘（43 秒）、YouTube 公開
- [x] App Store 連結
- [ ] **1024×1024 app icon** —— `Sources/Resources/Assets.xcassets/AppIcon.appiconset/AppIcon.png`
- [ ] **截圖必須 1179×2556、無裝置外框** —— 現有的是 1320×2868 和 2064×2752，**不符，要重產**
- [x] 免費試用或 promo code —— 全功能免費，已在〈A note for judges on testing〉說明

**Peace Prize**

- [x] 「how the app was designed to benefit individuals, the community, or society」的描述 —— 已寫在〈Why this benefits people〉

**不爭取的獎項**

- #BuildInPublic —— 沒有開發期間的公開分享，只有上架貼文，材料太薄
- Grand Prize —— 要上架後的成長數據
- Design Award —— 看動畫與美感，SideBell 是功能性設計
- HAMM —— 要轉換率／營收數字
- Catvertising / Next Gen / Ship Kotlin —— 不符（無廣告／非學生／非 KMP）

## 送出前確認

- [ ] 團隊欄位選 solo
- [ ] Devpost 是否要求填 RevenueCat 相關資訊（SDK 使用方式、產品設定）
