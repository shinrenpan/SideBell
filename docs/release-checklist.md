...
線上教學在 `shinrenpan.github.io/static/SideBell/guide/`，中英兩版各 25 張
**實機**截圖。拍攝清單與注意事項見 `docs/guide-screenshots.md`。

**動到下列任何一處，就要回頭看教學：**

| 動到什麼 | 影響哪些截圖 |
|---|---|
| 免責畫面、角色選擇 | `p-01` `p-01b` `c-01` |
| 權限請求的時機或文案 | `p-02` `c-02` `c-03` |
| 患者端格子、連線狀態列 | `p-03` `p-12` `p-04` `p-05` `p-13` |
| 兩段式點擊、設定頁、編輯項目 | `p-06`～`p-11` `c-08` |
| 照顧者端清單、通知內容 | `c-04`～`c-07` `c-10` `c-11` `c-12` |

> 這一項容易被漏，因為**教學過時不會讓任何測試失敗**。它只會讓照顧者照著
> 一張不存在的畫面找按鈕，然後放棄。

同時檢查教學裡的**文字**有沒有跟著 UI 走。已經發生過三次：藍牙權限的觸發
時機、「無人回應」的實際字串、三分鐘逾時的適用範圍——三次都是憑截圖推論而
沒有回頭讀程式碼。**改了 UI 字串就 grep 一次教學頁。**
